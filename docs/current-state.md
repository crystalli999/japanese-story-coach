...
- Stage 3B creates a deterministic 36-question N5 placement diagnostic covering kana reading, vocabulary meaning, and grammar recognition.
- Stage 3C records every answer as learner evidence, updates skill-specific confidence, and links retakes to earlier diagnostic runs.
- Stage 3D selects the earliest unmastered grammar target whose prerequisites are ready, adds its resolved vocabulary and weak review concepts, and persists a 15-minute structured lesson packet locally.
- Stage 3E exposes learner creation, diagnostic, and lesson-planning commands. No external provider is called.
...
- No OCR, PDF/script import, DeepSeek call, complete learning UI, FSRS scheduling implementation, or story response parser exists.
- iMac availability and path are unknown.

## Verification

- 24 tests cover private paths, migrations, inventory safety, provider privacy, APKG inspection/import, normalization, rollback, provenance, deduplication, grammar validation, placement/retakes, learner-memory updates, prerequisite-aware planning, and idempotent refresh.
- Database integrity is `ok`; the private database is migrated through `004_learning_loop.sql` with zero known foreign-key violations.
